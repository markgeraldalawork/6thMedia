...
export default function InfiniteMenu({ items = [] }) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = index => {
      if (!items || items.length === 0) return setActiveItem(defaultItems[0]);
    ...
    return () => {
      ...
      // no explicit destroy for sketch to avoid exceptions
    };
  }, [items]);
  ...
  return (
    <div className="w-full h-full flex flex-col items-center gap-6 scroll-mt-20" id="services-menu">

          {/* Title above the sphere */}
    <h1 className="text-3xl text-orange font-semibold mb-8 text-center" data-aos="fade-up">
      Our Services
    </h1>
      ...
      <canvas
        ...
        ref={canvasRef}
        className="cursor-grab w-full h-[80vh] overflow-hidden outline-none active:cursor-grabbing"
        data-aos="fade-up"
      />

      {activeItem && (
        <div className="w-full max-w-[800px] px-4 flex flex-col items-center text-center gap-4">
          <h2 className="select-none font-black text-3xl md:text-5xl">{activeItem.title}</h2>

          <p className="select-none text-lg md:text-2xl">{activeItem.description}</p>

          {/* <button
            onClick={handleButtonClick}
            className="mt-4 w-[60px] h-[60px] grid place-items-center bg-[#00ffff] border-[4px] border-black rounded-full cursor-pointer transition-all"
            aria-label="Open item"
          >
            <span className="text-[#060010] text-2xl">↗</span>
          </button> */}
        </div>
      )}
    </div>
